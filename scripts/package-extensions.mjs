...
console.log('  Chrome:  chrome://extensions -> enable Developer mode -> Load unpacked -> point at dist/');
console.log('           (or drag the .zip onto the page)');
console.log('  Firefox (any of):');
console.log('    a) Drag-drop on stock Firefox: requires a SIGNED xpi, run "npm run sign" after setting AMO_JWT_ISSUER / AMO_JWT_SECRET');
console.log('    b) about:debugging#/runtime/this-firefox -> "Load Temporary Add-on" -> select dist/manifest.json (works on stock, lost on restart)');
console.log('    c) Firefox Developer Edition / Nightly with about:config xpinstall.signatures.required=false -> drag the .xpi');
